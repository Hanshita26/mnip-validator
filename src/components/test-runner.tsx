...
// shadcn/ui reusable components for design
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { ScrollArea } from "@/components/ui/scroll-area"
...
export function TestRunner() {
  ...
  // Only create the validator instance once
  ...
  const runTests = async () => {
    ...
    try {
      ...
      for (let i = 0; i < testCases.length; i++) {
        const testCase = testCases[i]
        const actual = validator.validateMPIN(testCase.mpin, testCase.demographics)
        const expectedReasons = (testCase.expected.weaknessReasons || []).slice().sort()
        const actualReasons = (actual.weaknessReasons || []).slice().sort()
        ...
        // Optional: show progressive updates, but this can be omitted for performance
        // await new Promise((resolve) => setTimeout(resolve, 100))
      }
      ...
      setSummary({ total: results.length, passed: passedCount, failed: failedCount })
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Comprehensive testing of MPIN validation logic with 20+ test cases
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <div>
                          Reasons:{" "}
                          {test.expected.weaknessReasons?.join(", ") || "None"}
                        </div>
                      </div>
                      <div>
                        <div className="font-medium text-gray-700">Actual:</div>
                        ...
                        <div>
                          Reasons:{" "}
                          {test.actual?.weaknessReasons.join(", ") || "None"}
                        </div>
                      </div>
                    </div>

                    {test.demographics &&
                      Object.keys(test.demographics).some(
                        (key) =>
                          (test.demographics as any)[key] !== undefined &&
                          (test.demographics as any)[key] !== null &&
                          (test.demographics as any)[key] !== ""
                      ) && (
                        <div className="mt-2 text-xs text-gray-600">
                          Demographics: {JSON.stringify(test.demographics)}
                        </div>
